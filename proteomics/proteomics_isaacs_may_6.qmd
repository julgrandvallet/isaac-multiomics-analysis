...
proteomics_long <- scaled_data %>%
    as.data.frame() %>%
    rownames_to_column("Protein") %>%
    pivot_longer(-Protein, names_to = "Sample", values_to = "Abundance") %>%
    left_join(intensity_map %>% dplyr::rename(Sample = Sample), by = "Sample") %>%
    dplyr::select(Protein, Sample, Abundance, Sensitivity.group) %>%
    mutate(Group = Sensitivity.group)

# Step 2: Shapiro-Wilk normality test per protein and group
shapiro_results <- proteomics_long %>%
    ...
    mutate(normal_Low = shapiro_Low > 0.05, normal_Medium = shapiro_Medium > 0.05)

# Step 3: Levene’s test for equal variance
levene_results <- proteomics_long %>%
    group_by(Protein) %>%
    summarise(
        levene_p = tryCatch(leveneTest(Abundance ~ Group)$`Pr(>F)`[1], error = function(e) NA_real_),
        .groups = "drop"
    ) %>%
    mutate(equal_var = levene_p > 0.05)

# Step 4: IQR similarity between groups
iqr_results <- proteomics_long %>%
    group_by(Protein, Group) %>%
    summarise(IQR = IQR(Abundance), .groups = "drop") %>%
    pivot_wider(names_from = Group, values_from = IQR) %>%
    mutate(similar_shape = abs(Low - Medium) / pmax(Low, Medium) < 0.3)

# Step 5: Combine assumption checks
assumptions_summary <- shapiro_results %>%
    left_join(levene_results, by = "Protein") %>%
    left_join(iqr_results, by = "Protein") %>%
    mutate(
        use_ttest = normal_Low & normal_Medium & equal_var,
...
table(
    t_test = assumptions_summary$use_ttest,
    wilcox = assumptions_summary$use_wilcox
)
```

## Test de Welch, cuando tienes varianzas heterogéneas
...
#\* Add the 2 gene sets \# \* Plot the same genes in the NR CR

#Plot those specific genes to see if its significant or not wss2 Take each cell line and plot the expression for each individual one.
...
# distributions of protein intensities by group (Low vs Medium)

```{r}
# Long format + group annotation
proteomics_long <- intensity_matrix %>%
  rownames_to_column("Protein") %>%
  pivot_longer(-Protein, names_to = "Sample", values_to = "Abundance") %>%
  left_join(intensity_map %>% dplyr::select(Sample = colname, Group = Sensitivity.group), by = "Sample") %>%
  filter(!is.na(Group))  # Just in case
```
...
# 🧠 Step 1: Extract short sample names from 'colname' (lowercase + padded → uppercase no padding)
intensity_map_fixed <- intensity_map %>%
  mutate(Sample = str_extract(colname, "iv\\d+"),
         Sample = toupper(str_replace(Sample, "^IV0", "IV")))  # iv01 → IV1

# ✅ Inspect sample name alignment
cat("Sample names in intensity_matrix:\n")
print(colnames(intensity_matrix))
cat("\nSample names in intensity_map_fixed:\n")
print(intensity_map_fixed$Sample)

# 🔄 Step 2: Convert intensity_matrix to long format
proteomics_long_fmt <- intensity_matrix %>%
  as.data.frame() %>%
  rownames_to_column("Protein") %>%
  pivot_longer(-Protein, names_to = "Sample", values_to = "Abundance")

# 🧬 Step 3: Join with group info and compute log-abundance
proteomics_long_fmt <- proteomics_long_fmt %>%
  left_join(intensity_map_fixed %>% dplyr::select(Sample, Group = Sensitivity.group), by = "Sample") %>%
  filter(!is.na(Group)) %>%
  mutate(log_abund = log1p(Abundance))

# ✅ Confirm successful join
cat("\nPreview of long-format joined data:\n")
print(head(proteomics_long_fmt, 10))
cat("\nUnique groups:\n")
print(unique(proteomics_long_fmt$Group))

# 📊 Step 4: Compute group-wise stats
if (nrow(proteomics_long_fmt) > 0) {
  group_summary_stats_fmt <- proteomics_long_fmt %>%
    group_by(Group) %>%
    summarise(
      n = n(),
      n_zeros = sum(Abundance == 0),
      prop_zero = mean(Abundance == 0),
      mean_raw = mean(Abundance),
      sd_raw = sd(Abundance),
      skew_raw = e1071::skewness(Abundance),
      mean_log = mean(log_abund),
      sd_log = sd(log_abund),
      skew_log = e1071::skewness(log_abund),
      .groups = "drop"
    )

  cat("\n✅ Group-level summary statistics:\n")
  print(group_summary_stats_fmt)
} else {
  cat("\n⚠️ No data available after join. Still failing — sample names may not match.\n")
}
...
# 🔍 Density plots of raw abundance
ggplot(proteomics_long_fmt, aes(x = Abundance, fill = Group)) +
  geom_density(alpha = 0.4) +
  scale_x_log10() +
  labs(title = "Density Plot of Raw Abundance (log10-scaled)", x = "Abundance", y = "Density") +
  theme_minimal()

# 🔍 Density plots of log-transformed abundance
ggplot(proteomics_long_fmt, aes(x = log_abund, fill = Group)) +
  geom_density(alpha = 0.4) +
  labs(title = "Density Plot of log(Abundance + 1)", x = "log(Abundance + 1)", y = "Density") +
  theme_minimal()

# 📦 Boxplot of log-transformed abundance
ggplot(proteomics_long_fmt, aes(x = Group, y = log_abund, fill = Group)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.6) +
  geom_jitter(width = 0.15, size = 0.5, alpha = 0.2) +
  labs(title = "Boxplot of log(Abundance + 1)", y = "log(Abundance + 1)", x = "Group") +
  theme_minimal()

```

# 🧮 2. Check Assumptions (Levene’s test + Distribution fit checks)
...
# 🔎 Levene's Test for variance equality on log-abundance
leveneTest(log_abund ~ Group, data = proteomics_long_fmt)

# 📦 OPTIONAL: Fit distributions to one group only (e.g., Low)
log_vals_low <- proteomics_long_fmt$log_abund[proteomics_long_fmt$Group == "Low"]

# Fit to log-normal
fit_ln <- fitdist(log_vals_low, "norm")

# Fit to gamma
gamma_vals_low <- proteomics_long_fmt$Abundance[proteomics_long_fmt$Group == "Low"]
fit_gamma <- fitdist(gamma_vals_low[gamma_vals_low > 0], "gamma")  # Remove zeros
...
Goal: Load protein intensities and prepare spectral counts per gene.
...
# Load expression matrix (already log2-transformed here)
gene.matrix <- log2(intensity_matrix + 1)

# Read peptide-level table with spectral counts
peptide_file <- readxl::read_xlsx("MatthewWitkowski_4574840_CombinedModifiedPeptide.xlsx", col_names = TRUE)

# Identify Spectral Count columns (avoid "Unique" and "Intensity")
spectral_cols <- grep("Spectral Count$", names(peptide_file), value = TRUE)
spectral_cols <- spectral_cols[!grepl("Unique|Intensity", spectral_cols)]

# Ensure numeric type
peptide_file[spectral_cols] <- lapply(peptide_file[spectral_cols], as.numeric)
```

## 🔹 2. PSM Count Aggregation
Goal: Aggregate spectral counts per gene (min total per gene), align with intensity matrix.
```{r}
# Compute minimum total PSM count per gene
psm.count.table <- peptide_file %>%
  filter(!is.na(Gene)) %>%
  group_by(Gene) %>%
  summarise(count = min(rowSums(across(all_of(spectral_cols)), na.rm = TRUE)), .groups = "drop") %>%
  as.data.frame()

# Convert to named row format
rownames(psm.count.table) <- psm.count.table$Gene
psm.count.table$Gene <- NULL

# Align to expression matrix
psm.count.table <- psm.count.table[rownames(gene.matrix), , drop = FALSE]

# Add pseudocount (+1) to avoid zeros
psm.count.table$count <- psm.count.table$count + 1
```

##🔹 3. Model Design and Linear Fit (Limma)
Goal: Fit a linear model to the protein-level expression data (gene.matrix) to estimate log2 fold changes between groups (Medium vs Low) before DEqMS correction.


```{r}
# Remove duplicated genes
dups <- duplicated(rownames(gene.matrix))
if (any(dups)) {
  gene.matrix <- gene.matrix[!dups, , drop = FALSE]
}

# Define group and design matrix
group <- factor(intensity_map$Sensitivity.group, levels = c("Low", "Medium"))
design <- model.matrix(~0 + group)
colnames(design) <- levels(group)

# Fit limma model and contrast
fit1 <- lmFit(gene.matrix, design)
contrast <- makeContrasts(Medium - Low, levels = design)
fit2 <- contrasts.fit(fit1, contrast)
fit2 <- eBayes(fit2)
```

## 🔹 4. Prepare for DEqMS and Run
Goal: Attach count data to the limma fit and run DEqMS variance shrinkage.

```{r}
# Attach spectral count to limma model
fit2$count <- psm.count.table[rownames(fit2$coefficients), "count"]
valid_rows <- !is.na(fit2$count)

# Clean invalid rows in all fit2 components
# Filter all components of fit2 using valid row index
fit2_filtered <- lapply(fit2, function(comp) {
  if (is.matrix(comp) || is.data.frame(comp)) {
    if (nrow(comp) == length(valid_rows)) return(comp[valid_rows, , drop = FALSE])
  } else if (length(comp) == length(valid_rows)) {
    return(comp[valid_rows])
  }
  return(comp)
})
class(fit2_filtered) <- "MArrayLM"

```

Because proteins quantified with more PSMs provide more precise estimates, DEqMS models the relationship between variance and PSM count, and applies a Bayesian shrinkage adjustment that depends on this count. The idea is: low-PSM proteins get more shrinkage, high-PSM proteins get less.

```{r}
# Run DEqMS
fit3 <- spectraCounteBayes(fit2_filtered)
```
...
```{r}
# 3.1.1 Prior variance comparison
...
```{r}
# Residual plot: DEqMS vs Limma
op <- par(mfrow = c(1, 2), mar = c(4, 4, 4, 1), oma = c(0.5, 0.5, 0.5, 0))

# DEqMS residuals
Residualplot(fit3, xlab = "log2(PSM count)", main = "DEqMS")

# Limma residuals (based on deviation from prior)
x <- fit3$count
y <- log(fit3$s2.prior) - log(fit3$sigma^2)
plot(log2(x), y, ylim = c(-6, 2),
     ylab = "Variance (estimated - observed)",
     xlab = "log2(PSM count)",
     main = "Limma", pch = 20, cex = 0.5)
...
# Compare shrinkage of posterior variance
op <- par(mfrow = c(1, 2), mar = c(4, 4, 4, 1), oma = c(0.5, 0.5, 0.5, 0))

# Limma posterior variance
heatscatter(log2(fit3$count), log(fit3$s2.post),
            pch = 20, xlab = "log2(PSM count)",
            ylab = "log(Posterior Variance)",
            main = "Posterior Variance in Limma")

# DEqMS posterior variance
heatscatter(log2(fit3$count), log(fit3$sca.postvar),
            pch = 20, xlab = "log2(PSM count)",
            ylab = "log(Posterior Variance)",
...
```{r}
# Replace these indices with your sample columns for Low and Medium groups
low_idx <- which(intensity_map$Sensitivity.group == "Low")
med_idx <- which(intensity_map$Sensitivity.group == "Medium")

pval.ttest <- apply(gene.matrix, 1, function(x)
  t.test(as.numeric(x[low_idx]), as.numeric(x[med_idx]))$p.value)

logFC.ttest <- rowMeans(gene.matrix[, med_idx]) - rowMeans(gene.matrix[, low_idx])

ttest.results <- data.frame(
  gene = rownames(gene.matrix),
  logFC = logFC.ttest,
  P.Value = pval.ttest,
  adj.pval = p.adjust(pval.ttest, method = "BH")
)

ttest.results$PSMcount <- psm.count.table[ttest.results$gene, "count"]
ttest.results <- ttest.results[order(ttest.results$P.Value), ]

```

```{r}
ord.t <- fit1$coefficients[, 1] / fit1$sigma / fit1$stdev.unscaled[, 1]
ord.p <- 2 * pt(abs(ord.t), df = fit1$df.residual, lower.tail = FALSE)
ord.q <- p.adjust(ord.p, method = "BH")

anova.results <- data.frame(
  gene = rownames(fit1$coefficients),
  logFC = fit1$coefficients[, 1],
  t = ord.t,
  P.Value = ord.p,
  adj.P.Val = ord.q,
  PSMcount = psm.count.table[rownames(fit1$coefficients), "count"]
)

anova.results <- anova.results[order(anova.results$P.Value), ]
...
plot(sort(-log10(limma.results$P.Value), decreasing = TRUE),
     ...
     xlab = "All proteins ranked by p-value", col = "purple",
     main = "All Proteins")
...
legend("topright", legend = c("Limma", "DEqMS", "Anova", "t-test"),
       col = c("purple", "red", "blue", "orange"), lty = c(2, 1, 2, 2), lwd = 2, cex = 0.8)

# Plot 2: Top 500 proteins
plot(sort(-log10(limma.results$P.Value), decreasing = TRUE)[1:10],
     type = "l", lty = 2, lwd = 2, ylab = "-log10(p-value)", ylim = c(2, 10),
     xlab = "Top 500 proteins", col = "purple",
     main = "Top 500 Proteins")

lines(sort(-log10(DEqMS.results$sca.P.Value), decreasing = TRUE)[1:10],
      lty = 1, lwd = 2, col = "red")
lines(sort(-log10(anova.results$P.Value), decreasing = TRUE)[1:10],
      lty = 2, lwd = 2, col = "blue")
lines(sort(-log10(ttest.results$P.Value), decreasing = TRUE)[1:10],
      lty = 2, lwd = 2, col = "orange")
...
volcano_data <- DEqMS.results %>%
  ...
  mutate(
    GeneType = case_when(
      log2FC >= fc_cutoff & PValue <= pval_cutoff ~ "up",
      log2FC <= -fc_cutoff & PValue <= pval_cutoff ~ "down",
      TRUE ~ "ns"
    )
  )

# 🎨 Color and size aesthetics
cols <- c("up" = "#ffad73", "down" = "#26b3ff", "ns" = "gray80")
sizes <- c("up" = 2, "down" = 2, "ns" = 1)
alphas <- c("up" = 1, "down" = 1, "ns" = 0.5)
...
p <- ggplot(volcano_data, aes(
  ...
  geom_vline(xintercept = c(-fc_cutoff, fc_cutoff), linetype = "dashed", color = "gray40") +
  scale_fill_manual(values = cols, name = "Regulation") +
  scale_size_manual(values = sizes, guide = "none") +
  scale_alpha_manual(values = alphas, guide = "none") +
  labs(
    title = "Volcano Plot (DEqMS P-values)",
    subtitle = "Threshold: |log2FC| ≥ 1 & p-value ≤ 0.05",
    x = "log2(Fold Change) (Medium / Low)",
